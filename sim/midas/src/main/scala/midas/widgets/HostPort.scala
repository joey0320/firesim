...
class HostPortIO[+T <: Data](private val targetPortProto: T) extends Record with HasChannels {
  ...
  def bridgeChannels = {
    ...
    inputWireChannels.map({ case (field, chName) =>
      PipeBridgeChannel(chName, clockRT, Seq(), Seq(field.toNamed.toTarget), 1)
    }) ++
    outputWireChannels.map({ case (field, chName) =>
      PipeBridgeChannel(chName, clockRT, Seq(field.toNamed.toTarget), Seq(), 1)
    }) ++
    rvIns.map({ case (field, chName) =>
      ...
      ReadyValidBridgeChannel(
        fwdChName,
        revChName,
        getClock.toNamed.toTarget,
        Seq(),
        Seq(validTarget) ++ SimUtils.lowerAggregateIntoLeafTargets(field.bits),
        validTarget,
        field.ready.toNamed.toTarget
      )
    }) ++
