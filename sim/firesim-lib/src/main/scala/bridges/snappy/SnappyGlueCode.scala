...
class SnappyCompressCopyExpander(cp: CompressorParams)(implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    val memwrites_in  = Flipped(Decoupled(new CompressWriterBundle(cp)))
    val memwrites_out =         Decoupled(new CompressWriterBundle(cp))
  })

  val incoming_writes_Q = Module(new Queue(new CompressWriterBundle(cp), 4))
  incoming_writes_Q.io.enq <> io.memwrites_in


  val intermediate_writes_Q = Module(new Queue(new CompressWriterBundle(cp), 4))
 ...
 */
class SnappyCompressLitRotBuf(cp: CompressorParams)(implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    val memwrites_in = Flipped(Decoupled(new CompressWriterBundle(cp)))
  ...
  for (i <- 0 until NUM_QUEUES) {
    mem_resp_queues(i).enq.bits  := 0.U
    mem_resp_queues(i).enq.valid := false.B
    mem_resp_queues(i).deq.ready := false.B

    for (j <- 0 until NUM_QUEUES) {
      when (i.U === (write_start_index +& j.U) % NUM_QUEUES.U) {
        mem_resp_queues(i).enq.bits := incoming_writes_Q.io.deq.bits.data >> ((j.U) << 3)
      }
    }
  }
  ...
  for (i <- 0 until NUM_QUEUES) {
    ...
    for (j <- 0 until NUM_QUEUES) {
      when ((i.U +& read_start_index) % NUM_QUEUES.U === j.U) {
        remapVecData(i)   := mem_resp_queues(j).deq.bits
        remapVecValids(i) := mem_resp_queues(j).deq.valid
        mem_resp_queues(j).deq.ready := remapVecReadys(i)
      }
    }
  }
...
class SnappyCompressLitLenInjector(cp: CompressorParams)(implicit p: Parameters) extends Module {
  val io = IO(new Bundle {
    val memwrites_in  = Flipped(Decoupled(new CompressWriterBundle(cp)))
    val memwrites_out =         Decoupled(new CompressWriterBundle(cp))
  })
  ...
  val intermediate_writes_Q = Module(new Queue(new CompressWriterBundle(cp), 10))
  val lit_rot_buf = Module(new SnappyCompressLitRotBuf(cp))
  ...
  lit_rot_buf.io.consumer.user_consumed_bytes := 0.U
