...
class LZ77HashMatcher(val cp: CompressorParams)(implicit p: Parameters) extends Module {
  val io = IO(new Bundle{
    val write_snappy_header = Input(Bool())

    val memloader_in = Flipped(new MemLoaderConsumerBundle(cp))
  ...
  hash_table.io.read_req.unhashed_input_key := 0.U
  hash_table.io.read_req.current_absolute_addr := 0.U

  history_buffer.io.read_req_in.valid := false.B
  history_buffer.io.read_advance_ptr.valid := false.B
  history_buffer.io.read_advance_ptr.bits.advance_bytes := 0.U

  // end of message addr reg management
  when (io.memloader_in.output_last_chunk &&
        (io.memloader_in.available_output_bytes === io.memloader_in.user_consumed_bytes) &&
        io.memloader_in.output_valid &&
        io.memloader_in.output_ready) {
  ...
  switch (compressorState) {
    is (sWriteUncompressedSizeVarint) {
      ...
      when (io.memwrites_out.ready && io.src_info.valid) {
        CompressAccelLogger.logInfo("LZ77 Write header for rqid: %d\n", rotating_request_id)
    ...
    is (sClockInHTRead) {
      when (io.memloader_in.output_last_chunk && (io.memloader_in.available_output_bytes < 4.U)) {
        ...
        when (io.memloader_in.output_valid && io.memwrites_out.ready) {
          ...
          CompressAccelLogger.logInfo("LZ77 EOB: fewer than 4 bytes left.\n")
        ...
        when (io.memloader_in.output_valid) {
          ...
          CompressAccelLogger.logInfo("LZ77 HT Read Issued: key 0x%x, addr 0x%x\n",
        ...
        when ((skip_bytes + 4.U) > io.memloader_in.available_output_bytes) {
          when (io.memloader_in.output_last_chunk) {
            ...
            when (io.memwrites_out.ready && io.memloader_in.output_valid) {
              CompressAccelLogger.logInfo("LZ77 EOB: fewer than skip_bytes(=%d) bytes left.\n", skip_bytes)
              compressorState := sWriteUncompressedSizeVarint
              history_buffer.io.read_advance_ptr.valid := true.B
            }
          } .otherwise {
          ...
          when (io.memwrites_out.ready && io.memloader_in.output_valid) {
            CompressAccelLogger.logInfo("LZ77 Advance by skip_bytes(=%d).\n", skip_bytes)
    ...
    is (sHistoryResultAvailable) {
      ...
      CompressAccelLogger.logInfo("LZ77 Overall comparison num_match_bytes: %d\n", num_match_bytes)
      ...
      when (num_match_bytes >= io.memloader_in.available_output_bytes) {
        ...
        when (!io.memloader_in.output_last_chunk) {
          ...
          when (io.memloader_in.output_valid && io.memwrites_out.ready) {
            CompressAccelLogger.logInfo("LZ77 Emitting copy and EOB: offset: %d, length: %d.\n",
        ...
        when (io.memloader_in.output_valid && io.memwrites_out.ready) {
          CompressAccelLogger.logInfo("LZ77 Emitting copy and continuing: offset: %d, length: %d.\n",
