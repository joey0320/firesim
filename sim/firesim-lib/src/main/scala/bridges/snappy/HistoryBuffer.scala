 ...
 * to backpressure.
 */
class HistoryBufferSRAM()(implicit p: Parameters) extends Module {
  ...
  val LZ77HistBufOverProvisionFactor = 2
  val HIST_BUF_WIDTH = 32
  val HIST_BUF_ELEMS_PER_CHUNK = 4 * 512 * LZ77HistBufOverProvisionFactor
  ...
  for (i <- 0 until HIST_BUF_WIDTH) {
    read_indexing_vec(i) := 0.U
    read_ports_vec(i)    := 0.U
  }
  ...
  for (i <- 0 until HIST_BUF_WIDTH) {
    write_indexing_vec(i) := 0.U
    write_ports_vec(i)    := 0.U
    write_ports_write_enable(i) := false.B
  }
  ...
  for (elemno <- 0 until HIST_BUF_WIDTH) {
    recent_history_vec_next(elemno) := 0.U
